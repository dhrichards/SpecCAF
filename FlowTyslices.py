...
if y0:
    strainvec = np.linspace(0,1,100)
else:
    strainvec = np.linspace(0,0.5,100)
Tvec = np.linspace(-30,-5,3)
#Wvec = np.linspace(0,1,4)
Wvec = np.logspace(-1,1,5)
#Wvec = array([0.1, 0.5, 1, 3, 10])

Sgrid,Tgrid=np.meshgrid(Wvec,Tvec)
A=np.zeros((Tvec.size,Wvec.size,strainvec.size))
...
for i in range(Tvec.size):
    for j in range(Wvec.size):
        ...
        if y0:
            con=ax.contourf(strainvec,th*180/np.pi,rh,50,vmin=0,vmax=vm,cmap=colmap)
...
cbar=plt.colorbar(cm.ScalarMappable(norm(0, vm),cmap=colmap),cax=cbar_ax,ticks=np.mgrid[0:vm:0.1],orientation='horizontal')
cbar.set_label('$\\rho^*$')
cbar.solids.set_edgecolor("face")
plt.show()

fig.savefig('fig06' + paramtype +'.pdf',format='pdf',bbox_inches='tight')
